...
export function ContractorView({
  ...
  const canSubmit = !pendingClaim && status !== "COMPLETED" && totals.remaining > 0;

  const fillPct = Math.round(
    claimAmount && !isNaN(Number(claimAmount))
      ? (Math.min(Number(claimAmount), totals.remaining) / item.contractValue) * 100
      : 0,
  );
  ...
  function onSubmitClaim() {
    ...
    if (!amountRes.success) { setClaimErr(amountRes.error.issues[0].message); return; }
    const descRes = claimDescSchema.safeParse(claimDesc);
    if (!descRes.success) { setClaimErr(descRes.error.issues[0].message); return; }
    const claim = submitClaim(item.id, { amount: amountRes.data, description: descRes.data });
    setClaimAmount("");
    setClaimDesc("");
    ...
    <div className="space-y-4">
      ...
      {item.paymentConfirmationPending && (
        <div className="rounded-lg border border-status-submitted-fg/30 bg-status-submitted/30 p-3 space-y-2.5">
          ...
          <div className="text-sm text-muted-foreground">
            {item.approverName} has marked this payment as paid. Please confirm or dispute.
          </div>
            ...
            <button
              onClick={() => {
                ...
                toast.error("Payment disputed", {
                  description: "Payment marked as not received. The approver has been notified.",
                });
              }}
      ...
      {!item.paymentConfirmationPending && (
        <div className="rounded-lg border border-border p-3 space-y-1">
          ...
          <div className="text-sm font-medium">
            {item.paymentStatus.charAt(0) + item.paymentStatus.slice(1).toLowerCase()}
          </div>
        </div>
      )}
      ...
      {canSubmit && (
        <div className="rounded-lg border border-border p-3 space-y-2.5">
          ...
          <div>
            <label className="text-[11px] text-muted-foreground">Amount (EUR)</label>
            <input
              type="number"
              inputMode="decimal"
              min={0}
              ...
              value={claimAmount}
              onChange={(e) => { setClaimAmount(e.target.value); setClaimErr(null); }}
              placeholder="0"
            ...
            />
            {fillPct > 0 && (
              <div className="mt-1.5 text-[11px] text-muted-foreground">≈ {fillPct}% of contract</div>
            )}
          </div>
          <div>
            <label className="text-[11px] text-muted-foreground">Description / scope of work</label>
            <textarea
              value={claimDesc}
              onChange={(e) => { setClaimDesc(e.target.value); setClaimErr(null); }}
              rows={3}
              maxLength={500}
          ...
          >
            <Send className="h-4 w-4" /> Submit claim to {item.approverName.split(" ")[0]}
          </Button>
        </div>
      )}
      ...
      {pendingClaim && (
        <div className="rounded-md border border-status-submitted-fg/30 bg-status-submitted/40 p-3 text-sm text-status-submitted-fg flex items-start gap-2">
          ...
          <div>
            Claim #{pendingClaim.sequence} ({fmtCurrency(pendingClaim.amount)}) is awaiting
            approval from {item.approverName}. You can submit another once a decision is made.
          </div>
        </div>
      )}
    </div>
